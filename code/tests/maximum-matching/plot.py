...
def parse_input(x):
    match = re.match(r'input/(\d+)-(\d+)random(\d+)', x)
...
def generate_plots(csv_path):
    ...
    df['Matching'] = df['Input'].str.extract('-(\d+)')[0].astype(int)
    
    # Individual plots for each vertex count and matching count
    vertex_groups = df.groupby(['Vertices', 'Matching'])
    for graph, group in vertex_groups:
        ...
        vertices, matching = graph
        plt.title(f'Average Time for {vertices} Vertices with Matching Number <= {matching}')
        ...
        output_filename = os.path.join('benchmark_plots', f'maximum_matching_v{vertices}_m{matching}.png')
    ...
    x_labels = [f"{parse_input(x)[0]}-{-parse_input(x)[1]} (test{parse_input(x)[2]+1})" for x in unique_inputs]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join('benchmark_plots', 'maximum_matching_plot.png'))
    plt.close()

# Usage
generate_plots('maximum_matching.csv')
