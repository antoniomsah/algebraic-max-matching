...
\begin{enumerate}
  ...
  \item \textbf{DeleteEdgesWithin (Line 5)}: From \eqref{alg:delwithin}, takes \(g(|V|) = O(n^\omega)\) time;
...
In total there were 50 different test cases and each test case was executed 50 times to ensure behavior.
...
Even though the Edmonds-Blossom time complexity is theoricatilly worse than Harvey's algorithm, when \(m > n\) (which is true for all test cases with \(n = 200\)), 
it demonstrates a superior performance in all cases.
...
\begin{figure}[h]
  ...
  \caption{Perfect Matching benchmark with all test cases.}
  \label{fig:perf_matching}
\end{figure}

A more detailed case by case analysis follows.

In \cref{fig:v10-20}, the Rank-two algorithm outperforms Harvey's algorithm, likely due to its lower constant factor. 
